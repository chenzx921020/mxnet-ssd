...
class PascalVoc(Imdb):
    ...
    def __init__(self, image_set, devkit_path, shuffle=False, is_train=False,
        ...
        lines = open(devkit_path).readlines()
        #if lines:
            #print 'read txt ok'
        if self.is_train:
            self.labels = self._load_image_labels(lines)

    @property
    def cache_path(self):
        """
        make a directory to store all cachReturns:
        ---------
            cache path
        """
    ...
    def _load_image_set_index(self, shuffle):
        ...
        """
        image_set_index_file = self.devkit_path
        #assert os.path.exists(image_set_index_file), 'Path does not exist: {}'.format(image_set_index_file)
        with open(image_set_index_file) as f:
            image_set_index = [x for x in f.readlines()]
        if shuffle:
    ...
    def image_path_from_index(self, index):
        ...
        name = self.image_set_index[index].split()[0]
        image_file = os.path.join(self.data_path.strip('json2txt.txt'),name)
    ...
    def _label_path_from_index(self, img_url):
        """
        ...
        """
        img_path = os.path.join(self.data_path.strip('json2txt.txt'),img_url)
        print img_path
        label_file = cv2.imread(img_path)
        #assert os.path.exists(label_file), 'Path does not exist: {}'.format(label_file)
        return label_file

    def _load_image_labels(self,lines):
        """
        ...
        for idx in range(0,len(lines)):
            ...
            label_file = self._label_path_from_index(tmp[0])
            #print label_file
            #tree = ET.parse(label_file)
            #root = tree.getroot()
            #size = root.find('size')
            width = label_file.shape[1]
            height = label_file.shape[0]
            label = []
            ...
            #     continue
                ...
                xmin = float(tmp[k])/width
                if xmin <= 0 :
                    xmin = 1/width
                ymin = float(tmp[k+1])/height
                if ymin <= 0:
                    ymin = 1/height
                xmax = float(tmp[k+2])/width
                if xmax <=0:
                    xmax = 1/width
                ymax = float(tmp[k+3])/height
                if ymax <=0:
                    ymax = 1/height
